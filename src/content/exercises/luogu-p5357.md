...
hints:
  - |-
    AC 自動機 = Trie + fail 指標。fail[v] 指向「v 所代表字串的最長真後綴，且該後綴也是某個模式串的前綴」——這正是 KMP 的 border 概念推廣到多個模式串。
  - |-
    建 fail 時用 BFS。對每個節點的每個字元：子節點存在就讓它的 fail 指向「父節點的 fail 沿同一字元走一步」；**子節點不存在時把 next[c] 直接指到那一步**（路徑壓縮）。有了這步，掃文本時就永遠不必回退，每個字元只走一步。
  - |-
    樸素統計是：掃文本時每到一個節點就沿 fail 鏈往上，把沿途所有模式串結尾都加一。但 fail 鏈可能很長，最壞退化成 O(文本長 × 模式串長)——這正是「二次加強版」要卡掉的做法。
  - |-
    正解是把 fail 指標看成一棵**樹**（每個節點的父親是它的 fail）。掃文本時只在「當前節點」加一，最後在 fail 樹上做一次由葉到根的累加，每個節點的總和就是它代表的字串出現次數。
  - |-
    不必真的建樹再 DFS：**反著走 BFS 順序**就等於 fail 樹的由深到淺，一個迴圈搞定。這是這題最漂亮的一步。
solution_outline: |-
