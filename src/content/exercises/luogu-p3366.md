...
hints:
  - 把所有邊依權重遞增排序；只選連接不同連通塊的邊。
  - 用含路徑壓縮與按大小合併的並查集維護目前連通塊。
  - 選滿 n−1 條即完成；否則原圖不連通。邊權總和使用 long long。
core_knowledge: [Kruskal, 並查集, 最小生成樹切割性質]
judgment: 稀疏無向圖求最小生成樹，且題目要求在不連通時輸出固定字串。
solution_outline: |-
...
common_errors: [未判斷圖不連通, 權重總和使用 int 溢位, 把 orz 的大小寫或拼字輸出錯誤]
complexity:
  time: 'O(m log m)，排序主導'
  space: 'O(n + m)'
cpp_skeleton: |
...
source_book_pages: [659, 660]
source_pdf_pages: [289, 290]
