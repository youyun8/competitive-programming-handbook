...
hints:
  - |-
    每個版本存一份完整陣列是 O(nm) 空間，n、m 到 10^6 完全放不下。可持久化的核心觀察是：一次單點修改只影響**根到該葉的那條路徑**，其餘子樹完全沒變。
  - |-
    所以用線段樹存陣列，修改時只新建那條路徑上的 O(log n) 個節點，其他子節點指標直接**沿用舊版本**。這就是「路徑複製」，總空間 O((n + m) log n)。
  - |-
    每個版本只需記住它的根節點編號。查詢就是從指定版本的根往下走到葉。
  - |-
    實作上有個容易踩的坑：不要把 `nodes[x].left` 以參考傳進遞迴函式，因為遞迴裡的 `push_back` 可能讓 vector 重新配置，那個參考就懸空了。先取出區域變數、遞迴、再寫回。
  - |-
    本題的查詢操作也會產生新版本，內容與被查詢的版本相同——直接讓新版本的根指向舊根即可，成本 O(1)。
solution_outline: |-
