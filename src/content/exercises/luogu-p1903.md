...
hints:
  ...
  - |-
    套用一次修改時，只有當「被修改的位置目前落在窗口 [left, right] 內」才需要同步更新計數；否則只要改陣列本身。這個判斷漏掉是帶修莫隊最常見的錯誤。
  - |-
    撤銷修改需要知道舊值。實作上在套用時把舊值存進該筆修改的紀錄裡，撤銷時再讀出來——這樣同一筆修改可以反覆套用與撤銷。
solution_outline: |-
