...
prerequisites:
  - gcd-lcm
statement: 給定 n 個整數 a_i，可任取整數係數 x_i，求 |Σ a_i x_i| 能取得的最小正整數。
constraints:
  - 1 <= n <= 20
  - '|a_i| <= 10^9'
input_format: 第一行為 n；第二行為 n 個整數 a_i。
output_format: 輸出所有整數線性組合可得到的最小正值。
samples:
  - input: |
      2
      4 6
    output: |
      2
    explanation: 4 與 6 的最大公因數是 2，例如 4×(-1) + 6×1 = 2；而 1 無法表示，因為任何 4x+6y 都是偶數。 本示例依官方輸入輸出格式設計。
hints:
...
solution_outline: 答案就是所有 |ai| 的最大公因數。把結果初始化為 0（gcd 的單位元），依序對每個 |ai| 取 gcd 即可。單次 gcd 是 O(log)，整體 O(n log V)。
proof_or_invariant: '裴蜀定理保證 {Σ ai·xi : xi ∈ ℤ} 恰為 gcd(|a1|,...,|an|) 的倍數集合。可達性由擴展歐幾裡得構造，不可達性由「gcd 整除每個 ai，故整除任何線性組合」得出。因此最小正元素就是該 gcd。'
complexity:
  time: O(n log V)
  space: O(1)
cpp_skeleton: |
...
core_knowledge:
  ...
  - 最大公因數
  - 整數線性組合
judgment: 所有整數線性組合能得到的正整數恰為各係數最大公因數的倍數，因此答案是全部絕對值的 gcd。
common_errors:
  - 沒有對負係數取絕對值
  - 誤以為只能選非負係數
  - 把最小值或兩數 gcd 當作全部數的答案
---
