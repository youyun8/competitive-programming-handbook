...
kind: external-oj
difficulty: 3
topics: ['掃描線', '線段樹', '離散化', '矩形面積並']
prerequisites: ['segment-tree', 'discretization', 'scanline']
statement: >-
  給定 n 個座標軸對齊的矩形（各以左下角與右上角座標表示），求它們聯集的面積。本卡片的題意為本站依題目主題重新敘述；
  完整原文敘述與資料範圍請以外部題目頁面為準。
constraints:
...
samples:
  - input: |
    ...
    output: |
      7
    explanation: >-
      本站自製測資（本次工作環境無法連線原 OJ 取得官方範例）。兩個 2×2 正方形各佔 4，
      重疊部分是 1×1，聯集為 4 + 4 − 1 = 7。
hints:
  - 想像一條垂直線由左往右掃過平面。任一時刻它與所有矩形相交的部分是 y 軸上的一些區間；只要知道這些區間的聯集長度 L，線往右移動 Δx 就掃出 L·Δx 的面積。二維問題被壓成一維的「維護區間聯集長度」。
  - 每個矩形拆成兩個事件：在 x1 讓 y 區間的覆蓋次數 +1，在 x2 讓同一區間 −1。事件按 x 排序後依序處理。
  - y 座標先離散化，但要注意線段樹的葉子代表「相鄰兩個離散座標之間的區間」，不是座標點本身——因為我們要量的是長度，長度屬於區間。這也是為什麼事件的 y 上界要寫成 hi - 1。
  - 節點維護兩個量：cover（完全覆蓋此節點區間的矩形數）與 len（此區間內被覆蓋的長度）。因為 +1 與 −1 必定成對且作用在完全相同的節點集合上，cover 永不為負，所以完全不需要 lazy pushdown，回溯時重算 len 就好。
  - 迴圈順序必須是「先用上一段寬度結算面積，再套用當前事件」。寫反會整條算錯。另外先過濾掉 x1 == x2 或 y1 == y2 的退化矩形，否則 hi - 1 可能讓 size_t 回繞。
solution_outline: >-
  收集所有 y1、y2 排序去重成 ys，線段樹葉子數為 ys.size() - 1，葉子 i 代表區間 [ys[i], ys[i+1])。
  每個矩形產生事件 (x1, lo, hi-1, +1) 與 (x2, lo, hi-1, -1)，其中 lo、hi 是 y1、y2 在 ys 中的位置；
  跳過退化矩形。事件按 x 遞增排序後掃過：先 area += len[1] * (x_i − x_{i−1})，再套用事件更新。
  更新時只在「查詢區間完整包含節點區間」時累加 cover，回溯時依 cover 是否為正、是否為葉子重算 len。
proof_or_invariant: >-
  cover[u] 恰等於「以 u 為完整覆蓋單位」的矩形數，任一 y 座標點的覆蓋次數等於根到該葉路徑上所有 cover 之和。
  len[u] 的正確性可歸納：cover[u] > 0 時整段被覆蓋故取全長；否則沒有矩形完整蓋住 u，被覆蓋部分只能來自子節點，取兩者相加；葉子為基底。
  每個 +1 都有對應的 −1 且作用在完全相同的節點集合上，故 cover 恆非負，不會出現需要下推標記才能算對子孫的情況。
  事件按 x 排序後相鄰事件之間覆蓋長度為常數，累加 L·Δx 即為聯集面積；同一個 x 上多個事件時 Δx = 0 不貢獻，處理順序不影響結果。
complexity:
  time: O(n log n)
  space: O(n)
cpp_skeleton: |
