...
constraints:
  - '邊權為正整數，詢問的長度上限不大（本解以 10^7 為界）'
  - '節點數與詢問數都不小，不能對每對點各算一次距離'
  - '完整限制條件請參閱外部題目頁面'
...
hints:
  - |-
    樹上路徑問題的通用切法：選一個點當「分治中心」，把路徑分成**經過它**與**完全在某棵子樹內**兩類。後者遞迴處理，於是只要會算前者就行。
  - |-
  ...
  - |-
    距離集合用一個大布林陣列 `achievable[]` 標記即可 O(1) 查詢。但**千萬不要每層都 memset 整個陣列**——只記錄本層改動過的位置，離開時只清那些。這是點分治最常見的複雜度陷阱。
  - |-
    別忘了長度為 0 的「空半段」：`achievable[0] = 1`，這樣才能配出「一端就是重心」的路徑。
solution_outline: |-
