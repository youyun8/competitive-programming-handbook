...
core_knowledge:
  ...
  - 可持久化權值線段樹
  - 前綴版本相減
judgment: 查詢完全離線且序列不修改；每個區間的權值頻率可由兩個前綴版本相減，因此用主席樹把每次查詢降為對值域的一次二分。
statement: |-
...
hints:
  ...
  - |-
    區間 [l, r] 的權值分佈 = 第 r 棵樹「減去」第 l−1 棵樹。因為線段樹的計數可加減，兩棵樹**同步往下走**、相減就得到區間的計數。
solution_outline: |-
  離散化數值後，對每個前綴用可持久化插入建出權值線段樹，第 i 棵由第 i−1 棵路徑複製而來。查詢時把第 l−1 棵與第 r 棵一起往下走，用兩者的左子樹計數差決定往左或往右，走到葉即為第 k 小的離散下標，再反查原值。
proof_or_invariant: |-
  不變量是「第 i 棵樹的每個節點的計數，等於前綴 [1, i] 中落在該節點值域內的元素個數」。因為計數對前綴可加減，兩棵樹逐節點相減後恰為區間 [l, r] 的權值分佈，於是在其上二分即可求第 k 小。
complexity:
  time: '建樹 O(n log n)，單次查詢 O(log n)'
  space: 'O(n log n)'
common_errors:
  - 把原值直接當線段樹下標，忘記先離散化
  - 查詢時沒有同步移動第 r 與第 l-1 個版本
  - 往右子樹時忘記從 k 扣除左半的元素數
cpp_skeleton: |
  #include <bits/stdc++.h>
  using namespace std;
  ...
  int main() {
      ...
      //   的權值分佈；在這個「差」上二分：左子樹的計數差若 >= k 就往左走，
...
cpp_solution: |
  ...
  // 第 i 棵與第 i−1 棵只差一條路徑。兩棵相減就得到區間 [l, r] 的權值分佈，
  // 於是可以在樹上二分找第 k 小。
