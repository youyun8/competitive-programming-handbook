...
solution_outline: |-
  先依 (a, b, c) 排序並把完全相同的元素合併成帶計數的項目。CDQ 分治：遞迴左右兩半後，用雙指標依第二維合併，左半按第三維插入樹狀陣列、右半查詢前綴和，處理完逐項清空樹狀陣列並就地歸併。最後把每個項目的答案加上組內 count − 1 後統計分佈。
proof_or_invariant: |-
...
cpp_solution: |
  ...
  // 分治時「左半隻作為修改、右半隻作為查詢」，跨區間的貢獻就被一次算完。
  static int k_limit;
