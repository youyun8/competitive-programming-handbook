...
hints:
  ...
  - |-
    最後做一次 p 從 0 掃到 n 的掃描：邊掃邊維護前綴 [1, p] 的值域計數陣列，掃到 p 時統一回答掛在 p 上的所有請求。每個請求要對 [lo, hi] 內每個 x 查一次計數陣列，總量正是莫隊的總移動距離 O(n√n)。
  - |-
    兩個容易錯的地方：一是各次移動算出來的是**相鄰兩個詢問的差**，要照排序後的順序累加成前綴和才是答案，不能直接當成單一詢問的結果；二是 k = 0 時 a_x 與自己也會被計數陣列算進去，若 x ≤ p 要記得扣掉這個自配對。
solution_outline: |-
