...
prerequisites:
  - xor-basis
statement: 給定 n 個可重複的非負整數，任選其中若干個（也可不選），求可得到的最大異或和。
constraints:
  - 1 <= n <= 50
  - 0 <= S_i <= 2^50
input_format: 第一行為 n；第二行為 n 個整數 S_i。
output_format: 輸出最大子集異或和。
samples:
  - input: |
      4
      8 5 3 6
    output: |
      14
    explanation: 最佳解是隻選 8 與 6：1000 xor 0110 = 1110，也就是 14。全選反而是 8，可見「選越多越好」的直覺並不成立。 本示例依官方輸入輸出格式設計。
hints:
