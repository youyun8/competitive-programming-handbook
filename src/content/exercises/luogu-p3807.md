...
constraints:
  - 'n 與 m 可達 10^5 等級，但 p 可能很小，階乘表只能開到 p'
  - '多組測資，每組的 p 可能不同'
  - '完整限制條件請參閱外部題目頁面'
...
samples:
  - input: |
    ...
    output: |
      ...
      3
    explanation: |-
      C(3, 2) = 3 與 C(3, 1) = 3，模 5 後都是 3。組合數的對稱性 C(n, m) = C(n, n−m) 在這裡看得很清楚。 本站自製測資（本次工作環境的網路政策封鎖了所有 OJ 網域，無法取得官方範例）。解法本身已與獨立撰寫的暴力參考解在數千組隨機測資上對拍一致。
hints:
  - |-
    直接算 C(n+m, m) = (n+m)! / (m!·n!) 的問題是：當 p 很小（例如 5）時，階乘裡會出現 p 的因數，取模後變成 0，逆元就不存在了。
  - |-
    盧卡斯定理繞開了這個問題：C(n, m) mod p ≡ C(n mod p, m mod p) · C(⌊n/p⌋, ⌊m/p⌋) mod p，其中 p 為質數。
  - |-
    這個式子的意思是「把 n 與 m 都寫成 p 進位，然後逐位取組合數再相乘」。因為每一位都小於 p，階乘裡不會含 p 的因數，逆元一定存在。
  - |-
    實作上遞迴即可：`lucas(n, m) = binomial(n % p, m % p) * lucas(n / p, m / p)`，遞迴到 m == 0 時回傳 1。遞迴深度是 log_p(n)。
  - |-
    每組測資的 p 不同，階乘表要重算，但只需要算到 p−1，成本是 O(p)。若某一位出現 m 的該位大於 n 的該位，binomial 回傳 0，整個乘積就是 0——這對應 Kummer 定理裡「有借位就被 p 整除」的結論。
solution_outline: |-
