...
hints:
  - 每條原邊加入容量為 c 的正向邊與容量為 0 的反向邊，推流時同步更新兩者。
  - BFS 只走正殘量邊建立分層圖；DFS 只走 level 加一的邊以求阻塞流。
  - 以當前弧略過本輪已證明無法推流的邊；匯點不可達時由最大流最小割定理終止。
core_knowledge: [殘量網路, Dinic, 分層圖, 阻塞流]
judgment: 有向容量網路直接求單源單匯最大流，容量和需用 64 位元。
solution_outline: |-
...
common_errors: [漏建反向邊, 平行邊被覆寫而非分別加入, 容量與總流量使用 int, DFS 未使用當前弧]
complexity:
