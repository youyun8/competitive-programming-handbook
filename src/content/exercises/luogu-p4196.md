...
statement: >-
  給定 n 個凸多邊形，每個以逆時針順序列出頂點，求它們的公共部分面積。本卡片的題意為本站依題目主題重新敘述；
  完整原文敘述、資料範圍與輸出精度要求請以外部題目頁面為準。
constraints:
  - 每個凸多邊形的邊都對應一個半平面，總邊數決定複雜度
  - 輸入的凸多邊形為逆時針，可行側統一取有向邊的左側
  - 完整限制條件與輸出精度請參閱外部題目頁面
...
samples:
  - input: |
    ...
    output: |
      2.000
    explanation: >-
      本站自製測資（本次工作環境無法連線原 OJ 取得官方範例）。2×2 正方形與其左下半的直角三角形，
      交集就是那個三角形，面積 2。
hints:
  - 一個凸多邊形等於它所有邊的「左側半平面」之交。把 n 個多邊形的所有邊丟進同一組半平面，交集就是答案區域。
  - 沿著最終凸區域的邊界逆時針走一圈，會依序經過方向角遞增的邊。所以先按方向角排序，再由小到大增量加入。
  - 加入新半平面時，佇列尾端最後兩條的交點若已被新半平面切掉，那條尾端直線再也不可能成為邊界，彈掉它。因為角度繞一圈，頭端也會被切，所以要用雙端佇列，頭尾都檢查。
  - 方向相同的兩個半平面，較靠左者的可行區域是另一者的子集，只保留最靠左的那條。不去重的話 cross(a.v, b.v) 為 0，求交點會除以零得到 nan。
  - 全部加完後別忘了收尾：用頭條檢查尾端、用尾條檢查頭端，直到穩定。最後若佇列不足三條，代表沒有面積，輸出 0。
solution_outline: >-
