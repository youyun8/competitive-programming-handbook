...
hints:
  ...
  - |-
    左右端都落在同一區塊內的**短查詢**要特別處理：直接暴力掃一遍，成本 O(區塊大小)。若硬要套用主流程，右指標會需要往左移動，違反「不刪除」的前提。
  - |-
    維護每個值的最早與最晚出現位置。右端擴展時用「當前位置 − 最早位置」更新答案；左端臨時擴展時用「最晚位置 − 當前位置」更新。兩個方向的公式不同，別寫混。
solution_outline: |-
