...
prerequisites:
  ...
  - euler-totient
  - mobius-inversion
statement: 對每個詢問 n，同時求 Σ_{i=1}^n φ(i) 與 Σ_{i=1}^n μ(i)。
constraints:
  - 1 <= T <= 10
  - 1 <= n < 2^31
input_format: 第一行為測試組數 T；接著 T 行各有一個正整數 n。
output_format: 每組輸出一行兩個整數，依序為 φ 的前綴和與 μ 的前綴和。
samples:
  - input: |
      5
      1
    ...
    output: |
      ...
      3044 1
      303963552392 212
    explanation: n=8 時 φ(1..8) 為 1,1,2,2,4,2,6,4，和為 22；μ(1..8) 為 1,−1,−1,0,−1,1,−1,0，和為 −2。 本示例依官方輸入輸出格式設計。
hints:
