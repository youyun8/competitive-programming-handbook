...
hints:
  - |-
    後綴自動機的每個狀態代表一組「結束位置集合（endpos）完全相同」的子串，這組子串的長度恰好是一段連續區間 (len[link], len[state]]。所以狀態數是 O(n) 的，遠少於子串總數 O(n²)。
  - |-
    一個狀態的**出現次數**就是它的 endpos 集合大小。計算方法：建構時每個「原生」狀態（對應一個真實前綴）的計數設為 1，分裂出來的 clone 設為 0，最後沿 link 樹由深到淺累加。
  - |-
    clone 的計數必須是 0。它是為了維護 endpos 等價類而分裂出的虛擬節點，本身不對應任何一次真實出現——這是 SAM 最容易寫錯的一行。
  - |-
    沿 link 樹累加不必真的建樹再 DFS：狀態的 length 越大在樹上越深，所以**依 length 做計數排序**就得到由深到淺的順序，一個迴圈即可。
  - |-
    最後答案取所有「出現次數 > 1」的狀態中 len[state] × 出現次數的最大值。取 len[state]（該狀態能代表的最長子串）而不是更短的，因為同一狀態內所有子串的出現次數相同，長的一定更優。
solution_outline: |-
