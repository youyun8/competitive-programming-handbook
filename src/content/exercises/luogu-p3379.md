...
constraints:
  - 'n 與 m 都很大，單次詢問要 O(log n)'
  - '樹可能退化成一條長鏈，遞迴建表有爆棧風險'
  - '完整限制條件請參閱外部題目頁面'
...
hints:
  ...
  - |-
    第二步從大到小枚舉 j，**只在 up[j][a] != up[j][b] 時才跳**。這樣兩點始終停在 LCA 的嚴格下方，最後 up[0][a] 就是 LCA。
  - |-
    為什麼是「不相同才跳」？因為相同代表跳過頭了（可能越過 LCA）。不相同表示還沒到，跳過去仍然安全。這個貪心讓每一位各判斷一次，共 O(log n)。
solution_outline: |-
