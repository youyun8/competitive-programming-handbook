...
prerequisites:
  - 整除與模運算
statement: 兩隻青蛙從 x、y 出發，以 m、n 的步長在長度 L 的環上前進，求最早相遇時間或 Impossible。
constraints:
  - 0<=x,y,m,n<=2*10^9，1<=L<=2*10^9
...
cpp_skeleton: |+
  ...
  int main(){long long X,Y,m,n,L,x,y;cin>>X>>Y>>m>>n>>L;long long a=(m-n)%L,b=(Y-X)%L;if(a<0)a+=L;if(b<0)b+=L;long long g=ex(a,L,x,y);if(b%g)cout<<"Impossible\n";else{long long q=L/g,ans=x*(b/g)%q;cout<<(ans+q)%q<<"\n";}}
...
external_url: https://www.luogu.com.cn/problem/P1516
external_platform: Luogu
external_problem_id: P1516
external_title: 青蛙的約會
