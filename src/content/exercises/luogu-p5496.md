...
core_knowledge:
  ...
  - fail 指向最長迴文真後綴
  - fail 深度等於目前迴文後綴數
judgment: 輸入字元須以「上一個答案」即時解密；每個位置輸出其解密後前綴的迴文後綴個數。
hints:
  - 使用長度 -1 與 0 的兩個根，讓單字元與奇偶迴文共用擴展公式。
  - 加入字元時沿 fail 找第一個可兩側擴展的節點，不存在轉移才建立新迴文節點。
  - 新節點的 fail 深度為 fail 節點深度加一；輸出目前 last 的深度，再用它解密下一字元。
solution_outline: |-
...
common_errors:
  - 先解密完整字串而忽略強制在線
  - 單字元節點的 fail 未指向長度 0 根
  - 把本質不同迴文總數誤當目前位置的迴文後綴數
complexity:
  time: 'O(n)'
  space: 'O(n × 字元集)'
cpp_skeleton: |
