...
title: 洛谷 P5496 迴文自動機：每個位置的迴文後綴個數
chapter: 9
section: '9.4'
kind: external-oj
difficulty: 5
topics: ['迴文自動機', 'PAM', 'Eertree', '強制在線']
prerequisites: ['palindromic-tree', 'manacher']
statement: |-
  給定一個字串，對每個位置求出以該位置結尾的迴文子串個數。字串是加密的，每個字元要用上一個答案解密，因此必須線上處理。
...
output_format: '一行若干個整數，第 i 個表示以第 i 個位置結尾的迴文子串個數。'
samples:
  - input: |
      abbbaaaa
    output: |
      1 1 2 1 1 2 1 2
    explanation: |-
      字元是加密的，所以實際處理的字串並不是輸入看起來的樣子：依 (s[i] − 97 + last) mod 26 + 97 逐位解密後得到 accdbbcb，每個位置的迴文後綴個數即為 1 1 2 1 1 2 1 2。例如最後一位的 b 有 b 與 bcb 兩個迴文後綴。這也說明為什麼本題必須線上處理。 本站自製測資（本次工作環境的網路政策封鎖了所有 OJ 網域，無法取得官方範例）。解法本身已與獨立撰寫的暴力參考解在數千組隨機測資上對拍一致。
hints:
  - |-
    迴文自動機（又稱 Eertree）的每個節點代表一個**本質不同**的迴文串。關鍵事實是：長度為 n 的字串最多隻有 n 個本質不同的迴文子串，所以節點數是 O(n)。
  - |-
    結構有兩個根：長度 0 的偶根與長度 −1 的奇根。長度 −1 看起來很怪，但它讓「單一字元」也能套用同一條擴展公式 len = len[parent] + 2（−1 + 2 = 1），省掉大量特判。
  - |-
    fail[v] 指向 v 所代表迴文的**最長迴文真後綴**。加入新字元時，從 last 沿 fail 鏈往上找第一個能兩側擴展的迴文（也就是 s[i − len − 1] == s[i]），在它下面掛新節點。
  - |-
    本題要的答案是 depth[v]＝沿 fail 鏈到根的步數。這恰好等於「以當前位置結尾的迴文子串個數」，因為每個迴文後綴各對應 fail 鏈上的一個節點，一一對應不重不漏。
  - |-
    強制在線是刻意的設計：字元要用上一個答案解密，所以你不能先讀完整個字串再處理。這正好逼你寫出真正線上的 PAM——它本來就是逐字元擴展的結構，不像後綴陣列那樣需要看到全文。
solution_outline: |-
  建立奇偶兩個根。逐字元處理：先用上一個答案解密，再沿 fail 鏈找到第一個可兩側擴展的迴文作為父節點；若該轉移不存在就新建節點（length = len[parent] + 2，fail 由父節點的 fail 再找一次可擴展節點取得，depth = depth[fail] + 1）。輸出新 last 節點的 depth 即為該位置的答案。
proof_or_invariant: |-
  核心不變量是「last 恆為當前前綴的最長迴文後綴」。由此出發沿 fail 鏈即可枚舉所有迴文後綴，且鏈長等於迴文後綴個數，故 depth 就是答案。每個字元最多新增一個節點，配合 fail 鏈的攤還論證（與 KMP 相同），總時間為 O(n)。
complexity:
  time: 'O(n)'
  space: 'O(n × 字元集)'
cpp_skeleton: |
  ...
  // 迴文自動機（PAM / Eertree）：每個節點代表一個「本質不同」的迴文串。
  struct PalindromicTree {
      ...
      // TODO 1：沿 fail 鏈往上找到第一個能在兩側同時擴展的迴文，
      ...
      // TODO 2：加入一個字元並回傳「以此位置結尾的迴文子串個數」。
      ...
      //   「以當前位置結尾的迴文子串個數」，因為每個迴文後綴各對應一個節點。
      int add(char c) {
          text += c;
          (void)c;
          return 0;
...
cpp_solution: |
  ...
  // 迴文自動機（PAM / Eertree）：每個節點代表一個本質不同的迴文串，
  // fail 指向該迴文的最長迴文真後綴。節點的深度（沿 fail 到根的步數）
  // 恰好等於「以當前位置結尾的迴文子串個數」。
  struct PalindromicTree {
      struct Node {
          array<int, 26> next{};
      ...
      PalindromicTree() {
          nodes.resize(2);
          nodes[0].length = -1;  // 奇根：長度 -1，讓單字元迴文能掛上來
          nodes[0].fail = 0;
      ...
      int add(char c) {
          ...
          if (nodes[static_cast<size_t>(parent)].next[index] == 0) {
              ...
              if (nodes[static_cast<size_t>(created)].length == 1) {
                  nodes[static_cast<size_t>(created)].fail = 1;  // 單字元的迴文後綴是空串
              } else {
...
PAM 是專為迴文設計的自動機。長度 −1 的奇根與「depth 即答案」這兩個設計，值得畫一次小例子親手驗證。
