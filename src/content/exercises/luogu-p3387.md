...
hints:
  ...
  - |-
    縮點之後圖一定是 DAG（若還有環，環上的分量就該合併成一個，與分量的極大性矛盾）。DAG 上沒有環，最長路就可以用 DP 求。
  - |-
    Tarjan 求強連通分量：dfn 是進入時間、low 是「不經父邊能回到的最小 dfn」。走已訪問的點時，只有它**還在堆疊上**才能用它的 dfn 更新 low——不在堆疊上代表已屬於別的分量，用了會算錯。
  - |-
    當 low[u] == dfn[u] 時，u 是所在分量的根，把堆疊彈到 u 為止就得到一個完整的強連通分量。
  - |-
    DAG 最長路用拓撲排序遞推：`best[v] = max(best[v], best[u] + 分量點權)`，答案取所有 best 的最大值。注意起點不固定，所有入度為 0 的分量都要放進初始佇列。
solution_outline: |-
