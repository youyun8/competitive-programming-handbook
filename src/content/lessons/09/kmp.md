...
以下例題對應本章教材的前綴函數與迴文主題。題意皆為本站重新敘述，程式為獨立撰寫、可直接編譯的 C++17，讀完即得完整解法。
...
線性時間求最長迴文子串長度。先在字元間插入分隔符，把奇偶長度迴文統一處理，並加哨兵避免越界；維護目前右端最遠的迴文中心與右界，界內位置先用鏡像半徑初始化，只有超出右界的部分才需要新比較。右界只向右移，總時間 O(n)。

```cpp
#include <bits/stdc++.h>
using namespace std;

// Manacher：O(n) 求最長迴文子串長度。
...
對 `abacabad` 輸出 `7`：最長迴文是 `abacaba`。分隔符技巧讓奇偶長度迴文不必分開處理。
