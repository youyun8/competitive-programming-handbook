...
chapter: 9
section: '9.4'
title: 迴文樹（Eertree）：線性統計迴文子串
summary: 以兩個根節點與失配邊，線上建構並統計所有不同迴文子串。
...
需要在掃描字串過程中，即時得知目前為止的所有不同迴文子串，並支援統計出現次數、最長迴文後綴等資訊。

## 辨識題型的訊號

不同迴文子串個數、迴文出現次數、最長迴文後綴查詢、需要動態追加字元並查詢迴文結構。

## 核心想法與直覺

迴文樹有兩個虛擬根：長度 -1 的根（奇數長度迴文的父節點）與長度 0 的根（偶數長度迴文的父節點）。每個節點對應一個唯一的迴文子串，邊表示在兩端同時加上某字元。失配邊（suffix link）指向該迴文的最長真迴文後綴。

## 狀態／資料結構定義

每個節點記錄長度、出現次數、子節點映射、失配邊索引。`last` 指標維護目前最長迴文後綴對應的節點。

## 不變量或正確性證明

每次新增字元時，透過失配邊不斷回退，直到找到某個迴文可以在兩端加上新字元仍保持迴文。因為每次回退都跳到更短的迴文後綴，而字串總長度只增加 1，所以過程必定在常數步內結束。新增節點時再為它建一條失配邊，同樣沿失配邊回退即可找到正確目標。總節點數不超過 n+2，因此總時間 O(n)。

## 逐步演算法

1. 初始化兩個根節點，長度分別為 -1 與 0；
2. 逐字元掃描字串；
3. 從 `last` 出發沿失配邊回退，直到找到可延伸的迴文；
4. 若該迴文對應的子節點不存在則新建；
5. 更新新建節點的失配邊；
6. `last` 指向該節點並增加計數。
...
失配邊初始化錯誤（長度 1 的節點 link 應指向長度 0 的根）；字元集大小寫；回退條件寫錯導致無窮迴圈；長度 -1 的根用於奇數長度迴文的父節點。

## 與相似技巧的比較

Manacher 求每個中心的最長迴文半徑，但不直接列舉不同迴文子串。迴文樹則明確列出所有不同迴文並可統計出現次數，但只適用離線或線上追加場景。後綴自動機結合雜湊也能類似統計，但迴文樹更直接。

## 例題與分級練習

不同迴文子串個數、迴文出現次數、最長迴文雙後綴、迴文劃分計數。

## 本節重點速查

雙根處理奇偶長度；邊為兩端同加一字元；失配邊是最長真迴文後綴；均攤線性。
