...
learning_goals: [用 DFS 定義子樹狀態, 合併子節點結果, 掌握揹包型樹形 DP]
concepts: [tree-dp, subtree-dp, knapsack-on-tree]
complexity:
  time: O(n) 簡單樹形 DP，O(n × capacity) 揹包型
...
給定一棵樹，要求選點或選邊滿足條件；點之間有父子限制（如選父不能選子）。也常見「樹上揹包」選若干節點帶權重與價值。

## 核心想法與直覺

後序遍歷先處理子節點，再把子節點的 DP 數組合併到父節點。子樹之間互不幹擾，因此可以枚舉每個子節點的貢獻。
...
由於樹無環，子樹之間僅透過父節點相連。後序遍歷保證處理 u 時所有子樹結果已確定。枚舉子節點合併時類似揹包，保證每個子樹只被計入一次。
...
簡單樹形 DP（如獨立集）時間 O(n)，空間 O(n)。樹上揹包時間 O(n × capacity²)，可用優化降至 O(n × capacity)。
...
後序遍歷；子樹揹包合併；狀態含「選／不選」屬性；注意容量與無向邊的回溯。
