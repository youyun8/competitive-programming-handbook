...
對於積性函數 f，直接計算前綴和 S(n) = Σ\_{i=1}^{n} f(i) 需要 O(n) 時間。當 n 高達 10^11 時無法接受。杜教篩利用狄利克雷卷積與數論分塊，將時間降至亞線性。
...
Σ*{i=1}^{n} (f \* g)(i) = Σ*{d=1}^{n} g(d) · S(⌊n/d⌋)
...
- 進階：結合莫比烏斯反演，求 Σ*{i=1}^{n} Σ*{j=1}^{m} [gcd(i,j)=1]。
