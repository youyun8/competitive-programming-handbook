    ...
    merged.set(item.exerciseId, {
      ...
      lastPracticedAt: newest(
        current.lastPracticedAt
          ? { value: current.lastPracticedAt, updatedAt: current.lastPracticedAt }
          : undefined,
        item.lastPracticedAt
          ? { value: item.lastPracticedAt, updatedAt: item.lastPracticedAt }
          : undefined
      )?.value
    });
  }
...
export function mergeGuestAndCloud(local: GuestSnapshot, cloud: GuestSnapshot): GuestSnapshot {
  return {
    settings: newest<ReadingSettings>(local.settings, cloud.settings),
