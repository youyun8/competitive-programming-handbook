...
export const chapterExerciseSolutions: Record<string, ExerciseSolution> = {
  括號序列驗證: {
    ...
    correctness: 'stack 依序保存尚未配對的左括號。右括號只能與最近的未配對左括號相配，因此每次檢查頂端既必要也充分。',
  ...
  循環佇列: {
    ...
    correctness: '保留一格不用可讓空與滿具有不同表示；索引每次取模，所以元素的邏輯順序跨越陣列尾端後仍保持不變。',
  ...
  合併木材: {
    approach: '把所有長度放入小根堆。反覆取出最短的兩段，將其合併成本加入答案，再把長度和放回堆中。',
    correctness:
  ...
  第一個可行答案: {
    ...
    correctness: '單調性保證每次排除的一半不含第一個可行答案；迴圈結束時候選只剩 left，它就是最小可行值。',
  ...
  逆序對: {
    approach: '用合併排序分割序列。合併兩個已排序區間時，若右半元素先被取出，它會與左半所有尚未取出的元素形成逆序對。',
    correctness: '逆序對只分成完全位於左半、完全位於右半、跨越兩半三類；遞迴計算前兩類，合併步驟恰好計算第三類。',
    complexity: '時間 O(n log n)，額外空間 O(n)。'
  },
  網格連通塊: {
    approach: '逐格掃描；遇到尚未造訪的可走格就啟動一次 BFS/DFS，標記同一塊內所有格並累計面積。',
    correctness: '一次 flood fill 會沿所有合法相鄰邊走到且只走到同一連通塊；外層掃描因此每個連通塊恰好啟動一次。',
    complexity: '時間 O(hw)，空間 O(hw)。'
  },
  無權最短路: {
    approach:
      '從起點 BFS，在節點第一次入隊時設定 distance 與 parent。到達終點後沿 parent 反向走回起點，再反轉得到路徑。',
    correctness: 'BFS 按距離層遞增展開，所以節點第一次被發現時已使用最少邊數；parent 保存的正是該最短路徑上的前一點。',
    complexity: '時間 O(V+E)，空間 O(V)。'
  },
  零一邊權: {
    approach: '使用 deque 做鬆弛。權重 0 的改善節點放前端，權重 1 的改善節點放後端，使待處理距離保持不下降。',
    correctness: '從目前最小距離 d 出發只會產生 d 或 d+1；前推零權、後推一權等價於此特例上的 Dijkstra 排序。',
    complexity: '時間 O(V+E)，空間 O(V)。'
  },
  雙向密碼鎖: {
    approach:
      '從起點與終點各維護距離表，每輪展開 frontier 較小的一側。新狀態若已被另一側看見，就以兩側距離和更新答案。',
    correctness: '所有合法操作可逆，因此任一完整路徑都可在某個狀態分成兩段；按層擴展保證首次最短交會給出最少步數。',
    complexity: '最壞仍與狀態數線性，典型搜尋深度由 d 降為約 d/2。'
  },
  動態連通性: {
    approach: '以並查集保存每個集合代表。find 使用路徑壓縮，union 依大小或秩把較小樹接到較大樹；兩點代表相同即連通。',
    correctness: '每次 union 只合併指定的兩個等價類，不會拆散既有關係；find 回傳唯一代表，因此代表相等恰等價於同集合。',
    complexity: 'm 次操作為 O(m α(n))，空間 O(n)。'
  },
  區間和: {
    approach:
      '只有點更新與區間和時用 Fenwick Tree：add 更新涵蓋該點的 lowbit 區塊，prefix_sum 向前累加；區間和是兩個前綴差。',
    correctness: 'Fenwick 的查詢路徑把前綴拆成互斥的二進位區塊，更新路徑則恰好修改所有包含該點的區塊摘要。',
  ...
  模快速冪: {
    approach:
      '先把 base 正規化到 [0,mod)。維持 result*base^exponent 與原目標同餘；指數為奇數時乘入 result，之後 base 平方、指數除以二。乘法使用倍增取模，避免中間值溢位。',
    correctness: '奇數情況抽出一個 base，偶數情況用 base^(2q)=(base²)^q；每輪都保持不變量，指數歸零時 result 即答案。',
  ...
  質因數批次查詢: {
    approach: '線性篩同時保存每個數的最小質因數。分解 x 時反覆取 spf[x] 並除盡，即可得到質因數與次方。',
    correctness: '篩法讓每個合數由其最小質因數唯一生成一次；分解時每一步移除一個已證實的質因數，直到剩下 1。',
  ...
  字典統計: {
    ...
    correctness: '每個字串在 Trie 中對應唯一根到節點路徑，共同前綴共享同一路徑，所以節點計數恰等於具有該前綴的字串數。',
  ...
  字典序拓撲排序: {
    approach: '計算入度，把所有入度 0 的點放入小根堆。每次取編號最小者輸出，刪除其出邊並把新出現的入度 0 點加入堆。',
    correctness:
  ...
  橋與割點: {
    ...
    correctness: 'low[v] 是 v 子樹不經父邊可到達的最早祖先；上述不等式正好表示子樹沒有繞回 u 上方或 u 本身的替代路徑。',
